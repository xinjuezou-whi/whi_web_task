(function (global, factory) {
    ...
    const chatInit = () => {
        const {getData: e} = window.phoenix.utils, t = ".chat-sidebar", a = ".chat-textarea", c = "[data-chat-thread]",
            ...
            n = document.querySelector(a), d = document.querySelectorAll(c), l = document.querySelector(r),
    ...
    const detectorInit = () => {
        const {addClass: e} = window.phoenix.utils, {is: o} = window, i = document.querySelector("html");
    ...
    const togglePaginationButtonDisable = (e, t) => {
        ...
        if (window.List) {
            ...
            t.length && t.forEach((t => {
                ...
                a.pagination && (a = {
                    ...a,
                    pagination: {item: "<li><button class='page' type='button'></button></li>", ...a.pagination}
                });
                const l = t.querySelector('[data-list-pagination="next"]'),
                    n = t.querySelector('[data-list-pagination="prev"]'), s = t.querySelector('[data-list-view="*"]'),
                    o = t.querySelector('[data-list-view="less"]'), c = t.querySelector("[data-list-info]"),
                    r = document.querySelector("[data-list-filter]"), d = new List(t, a);
                d.on("updated", (e => {
                    const i = t.querySelector(".fallback") || document.getElementById(a.fallback);
                    i && (0 === e.matchingItems.length ? i.classList.remove("d-none") : i.classList.add("d-none"));
                }));
                const g = d.items.length, u = d.page, h = d.listContainer.querySelector(".btn-close");
                let b = Math.ceil(g / u), m = d.visibleItems.length, v = 1;
                h && h.addEventListener("search.close", (() => {
                    d.fuzzySearch("");
                }));
                const p = () => {
                    c && (c.innerHTML = `${d.i} to ${m} <span class='text-600'> Items of </span>${g}`), n && togglePaginationButtonDisable(n, 1 === v), l && togglePaginationButtonDisable(l, v === b), v > 1 && v < b && (togglePaginationButtonDisable(l, !1), togglePaginationButtonDisable(n, !1));
                };
                p(), l && l.addEventListener("click", (e => {
                ...
                if (s && s.addEventListener("click", (() => {
                    d.show(1, g), b = 1, v = 1, m = g, p(), w();
                })), o && o.addEventListener("click", (() => {
                    d.show(1, u), b = Math.ceil(g / u), v = 1, m = d.visibleItems.length, p(), w();
                })), a.pagination && t.querySelector(".pagination").addEventListener("click", (e => {
                    if ("page" === e.target.classList[0]) {
                        const t = Number(e.target.getAttribute("data-i"));
                        t && (d.show(u * (t - 1) + 1, m), m = d.visibleItems.length < u ? u * (t - 1) + d.visibleItems.length : u * t, v = t, p());
                    }
                })), a.filter) {
                    const {key: e} = a.filter;
                    r.addEventListener("change", (t => {
                        d.filter((i => "" === t.target.value || i.values()[e].toLowerCase().includes(t.target.value.toLowerCase())));
                    }));
                }
                if (i) {
                    window.phoenix.BulkSelect.getInstance(i).attachRowNodes(d.items.map((e => e.elm.querySelector("[data-bulk-select-row]")))), i.addEventListener("change", (() => {
    ...
    const handleNavbarVerticalCollapsed = () => {
        const {getItemFromStore: e, hasClass: t, setItemToStore: a, resize: o} = window.phoenix.utils, l = "html",
            r = "body", n = ".navbar-vertical", c = ".navbar-vertical-toggle", s = ".navbar-vertical .navbar-collapse",
            i = ".navbar-vertical .nav-link.active", v = "click", d = "mouseover", m = "mouseleave",
            u = "navbar.vertical.toggle", b = "navbar-vertical-collapsed", h = "navbar-vertical-collapsed-hover",
            g = document.querySelector(c), p = document.querySelector(l), y = document.querySelector(s),
            E = document.querySelector(i), L = e("phoenixIsNavbarVerticalCollapsed", !1);
        g && g.addEventListener(v, (e => {
            g.blur(), p?.classList.toggle(b), a("phoenixIsNavbarVerticalCollapsed", !L);
            const t = new CustomEvent(u);
            e.currentTarget?.dispatchEvent(t);
        })), y && (y.addEventListener(d, (() => {
            t(p, b) && p?.classList.add(h);
        })), y.addEventListener(m, (() => {
            t(p, h) && p?.classList.remove(h);
        })), E && !L && E.scrollIntoView({behavior: "smooth"}));
        const S = () => {
            const e = document.querySelector(r).offsetHeight, t = document.querySelector(n)?.offsetHeight;
            document.documentElement.classList.contains(b) && e < t ? document.documentElement.style.minHeight = `${t}px` : document.documentElement.removeAttribute("style");
        };
        S(), o((() => {
            S();
        })), g && g.addEventListener("navbar.vertical.toggle", (() => {
            S();
        }));
    };
    ...
    const phoenixOffcanvasInit = () => {
        const {getData: e} = window.phoenix.utils, o = document.querySelectorAll("[data-phoenix-toggle='offcanvas']"),
            t = document.querySelector("[data-phoenix-backdrop]"), n = e => {
                e.classList.remove("show"), document.body.style.removeProperty("overflow");
            };
        o && o.forEach((o => {
            const a = e(o, "phoenix-target"), c = document.querySelector(a),
                d = c.querySelector("[data-phoenix-dismiss='offcanvas']");
    ...
    const sortableInit = () => {
        const t = document.querySelectorAll("[data-sortable]"), e = document.querySelector("[data-kanban-container]");
        e && e.addEventListener("click", (t => {
            t.target.hasAttribute("data-kanban-collapse") && t.target.closest(".kanban-column").classList.toggle("collapsed");
        })), t.forEach((t => window.Sortable.create(t, {
            animation: 150,
            group: {name: "shared"},
            delay: 100,
            delayOnTouchOnly: !0,
            forceFallback: !0,
            onStart(t) {
                window.Sortable.ghost.style.opacity = 1, document.body.classList.add("sortable-dragging"), window.Sortable.ghost.querySelector(".dropdown-menu").classList.remove("show");
    ...
    const {config: config} = window.config, initialDomSetup = e => {
        ...
        const {getData: e, getItemFromStore: t} = window.phoenix.utils, o = new DomNode(document.body),
            a = document.querySelector(".navbar-vertical"), r = document.querySelector(".navbar-top"),
            n = document.querySelector(".support-chat-container");
        initialDomSetup(o.node), o.on("click", (i => {
            const c = new DomNode(i.target);
            if (c.data("theme-control")) {
                const l = c.data("theme-control");
                let d = i.target["radio" === i.target.type ? "value" : "checked"];
                switch ("phoenixTheme" === l && "boolean" == typeof d && (d = d ? "dark" : "light"), config.hasOwnProperty(l) && window.config.set({[l]: d}), window.history.replaceState(null, null, window.location.pathname), l) {
                    case"phoenixTheme": {
                        ...
                        const e = new CustomEvent("clickControl", {detail: {control: l, value: d}});
                        i.currentTarget.dispatchEvent(e), changeTheme(o.node);
                        break
                    }
                    case"phoenixNavbarVerticalStyle":
                        a.classList.remove("navbar-darker"), "default" !== d && a.classList.add(`navbar-${d}`);
                        break;
                    case"phoenixNavbarTopStyle":
                        r.classList.remove("navbar-darker"), "transparent" !== d && r.classList.add(`navbar-${d}`);
                        break;
                    case"phoenixNavbarTopShape":
                        "dual-nav" === t("phoenixNavbarPosition") ? el.setAttribute("disabled", !0) : (t => {
                    ...
                    case"phoenixSupportChat":
                        n?.classList.remove("show"), d && n?.classList.add("show");
                        break;
                    case"reset":
